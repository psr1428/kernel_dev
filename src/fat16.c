...
int fat16_get_total_items_for_directory(struct disk* disk, uint32_t directory_start_sector)
{
    ...
    if(diskstreamer_seek(stream, directory_start_pos) != KRNL_OK)
    {
        res = -EIO;
        goto out;
    }
    ...
    while(1)
    {
        if (diskstreamer_read(stream, &item, sizeof(item)) != KRNL_OK)
        {
            res = -EIO;
            goto out;
        }
...
int fat16_get_root_directory(struct disk* disk, struct fat_private* fat_private, struct fat_directory* directory)
{
    ...
    struct disk_stream* stream = fat_private->directory_stream;
    if (diskstreamer_seek(stream, fat16_sector_to_absolute(disk, root_dir_sector_pos)) != KRNL_OK)
    {
        res = -EIO;
        goto out;
    }

    if (diskstreamer_read(stream, dir, root_dir_size) != KRNL_OK)
    {
        res = -EIO;
        goto out;
    }
...
int fat16_resolve(struct disk* disk)
{
    ...
    if (diskstreamer_read(stream, &fat_private->header, sizeof(fat_private->header)) != KRNL_OK)
    {
        res = -EIO;
        goto out;
    }
    ...
    if (fat16_get_root_directory(disk, fat_private, &fat_private->root_directory) != KRNL_OK)
    {
        res = -EIO;
        goto out;
    }
...
{
    int res = 0;
    struct fat_file_descriptor *fs_private = private;
    struct fat_item *desc_item = fs_private->item;
    if(desc_item->type != FAT_ITEM_TYPE_FILE)
    {
        return -EINVARG;
    }
...
static void fat16_free_file_desc(struct fat_file_descriptor *private)
{
    fat16_fat_item_free(private->item);
}
int fat16_close(void *private)
{
    fat16_free_file_desc((struct fat_file_descriptor*)private);
    return 0;
}